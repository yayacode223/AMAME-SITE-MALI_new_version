...
): Promise<ConcoursResponsePaginated> => {
  const {
    page = 0,
    size = 9,
    sortBy = "dateLimite",
...
const createConcours = async (
  ...
  const concoursBlob = new Blob([JSON.stringify(request)], {
  ...
  const response = await Api.post<ConcoursResponse>(
    "/admin/concours",
    formData,
    {
      headers: {
...
const updateConcours = async (
  ...
  const response = await Api.put<ConcoursResponse>(
    `/admin/concours/${id}`,
    formData,
    {
      headers: {
...
export const downloadConcoursFile = async (
  filePath: string,
  concoursName: string,
) => {
  if (!filePath) return;
  ...
  try {
    ...
    link.setAttribute(
      "download",
      `${concoursName.replace(/\s+/g, "_")}_document.pdf`,
    );
...
export const concoursKeys = {
  ...
  lists: (params: ConcoursDefaultSearchParams) =>
    [...concoursKeys.all, "list", params] as const,
  detail: (id: number) => [...concoursKeys.all, "detail", id] as const,
  filter: (params: ConcoursFilterParams) =>
    [...concoursKeys.all, "filter", params] as const,
  search: (params: ConcoursFilterParams) =>
    [...concoursKeys.all, "search", params] as const,
};

// Hooks React Query - GET
export const useConcoursLists = (
  params: ConcoursDefaultSearchParams,
...
export const useConcoursDetail = (
  id: number,
  options?: { enabled?: boolean },
) => {
  return useQuery({
...
export const useConcoursFilter = (
  params: ConcoursFilterParams,
  options?: { enabled?: boolean },
) => {
  return useQuery({
...
export const useConcoursSearch = (
  params: ConcoursFilterParams,
  options?: { enabled?: boolean },
) => {
  return useQuery({
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: concoursKeys.detail(variables.id),
      });
    },
  });
};
