...
import { LoginType, RegisterResponse, RegisterPayload, RegisterUpdatePayload } from '@/types/userType';
...
export const register = async (data: RegisterPayload): Promise<RegisterResponse> => {
  ...
  const response = await Api.post<RegisterResponse>('/visitor/register', formData,{
     headers: {
...
export const update = async (data : RegisterUpdatePayload): Promise<RegisterResponse> => {
  const formData = new FormData(); 
  formData.append('user', new Blob([JSON.stringify(data.user)], {
    type: 'application/json',
  })); 
  ...
  const response = await Api.put<RegisterResponse>(`/user/update/${data.id}`, formData, {
    headers: {
      'Content-Type': null, //'multipart/form-data',
    }
  });
